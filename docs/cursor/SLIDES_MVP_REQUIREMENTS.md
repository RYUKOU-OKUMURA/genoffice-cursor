...
- **FR-023** `add_text_box` and `add_shape` create bounded native elements on a
  valid slide.
- **FR-024** `add_diagram` creates one supported native diagram layout with
  bounded node text and node count.
- **FR-025** Invalid indices, stale element IDs, NaN/infinite geometry, invalid
  colors, unsupported shape names, oversized text, and excessive calls fail
  before mutation.
...
- **FR-033** A user can save through the existing Slides save flow and reopen
  the PPTX with generated text and diagrams intact.
...
- **NFR-001 Security** The SDK runs in local mode with the sandbox enabled and
  ...
  web, task/subagent, or ambient MCP tool is available.
...
| A-03  | Open a blank presentation and ask for a title plus a three-step process diagram | Native text and a native diagram appear on the requested slide with no out-of-bounds element |
...
| A-08  | Save As, close, and reopen the generated PPTX                                   | Text, diagram, ordering, and editable native elements survive                                |
